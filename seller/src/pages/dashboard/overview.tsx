  ...
  return (
    <div className="space-y-4 text-sm">
      <div className="flex items-center justify-between">
        <h1 className="text-[16px] font-bold text-gray-900">Sales Overview</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPI icon={DollarSign} label="Revenue" value={loading ? '—' : `฿${formatMoney(revenue)}`} tone="text-green-700" />
        <KPI icon={ShoppingCart} label="Orders" value={loading ? '—' : totalOrders.toLocaleString()} tone="text-blue-700" />
        <KPI icon={ShoppingCart} label="Pending fulfillment" value={loading ? '—' : pending.toLocaleString()} tone="text-orange-700" />
        <KPI icon={AlertTriangle} label="Low stock items" value={loading ? '—' : lowStock.toLocaleString()} tone="text-red-700" />
      </div>

      <div className="rounded-lg border border-gray-100 px-5 py-4">
        <h3 className="text-[12px] font-bold text-gray-700 mb-2">All-time totals</h3>
