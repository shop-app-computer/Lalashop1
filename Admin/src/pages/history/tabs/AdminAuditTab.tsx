  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 px-4 py-3 bg-gray-50/50 text-[11px]">
        <div>
          <p className="text-gray-500 uppercase">{t('pages.history.totalEvents')}</p>
          <p className="text-base font-bold tabular-nums">{items.length}</p>
        </div>
        <div>
          <p className="text-gray-500 uppercase">{t('pages.history.uniqueAdmins')}</p>
          <p className="text-base font-bold tabular-nums">{new Set(items.map((i) => i.admin?._id).filter(Boolean)).size}</p>
        </div>
        <div>
          <p className="text-gray-500 uppercase">{t('pages.history.uniqueActions')}</p>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-[12px] tabular-nums">
          <thead className="text-[11px] text-gray-500 tracking-wide">
            <tr>
              <th className="px-4 py-2 text-left font-semibold">{t('table.auditId')}</th>
              <th className="px-4 py-2 text-left font-semibold">{t('table.adminIp')}</th>
              <th className="px-4 py-2 text-left font-semibold">{t('table.action')}</th>
              <th className="px-4 py-2 text-left font-semibold">{t('table.target')}</th>
              <th className="px-4 py-2 text-left font-semibold">{t('table.date')}</th>
            </tr>
          </thead>
          <tbody>
            {loading && (
            ...
            {!loading && !error && items.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-400 text-[12px]">{t('pages.history.noAuditEvents')}</td></tr>
            )}
          </tbody>
        </table>
      </div>
