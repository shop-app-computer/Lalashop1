...
const withdrawals: { id: number; userId: number; shopId: string; shopName: string; userTimestamp: string; fullName: string; bank: string; accountNo: string; phone: string; amount: string; adminUser: string; processedTimestamp: string; status: Status }[] = [
  { id: 700112451, userId: 1001, shopId: 'SHOP-1001', shopName: 'Lala Fashion', userTimestamp: '2024-04-23 14:30:12', fullName: 'Somsack Souvanna', bank: 'BCEL One', accountNo: '160-12-3456', phone: '020 5555 5555', amount: '5,000,000', adminUser: 'admin_alex', processedTimestamp: '2024-04-23 14:45:05', status: 'pending' },
  { id: 141111111, userId: 1002, shopId: 'SHOP-1002', shopName: 'Tech Gadgets', userTimestamp: '2024-04-23 09:15:44', fullName: 'Keo Viseth', bank: 'JDB Bank', accountNo: '751-01-9988', phone: '020 2222 3333', amount: '12,000,000', adminUser: 'admin_keo', processedTimestamp: '2024-04-23 09:30:11', status: 'approved' },
];
...
const SellerWithdrawals = () => {
  ...
  const filtered = withdrawals.filter(
    (w) => (filter === 'all' || w.status === filter) &&
      (!q || w.fullName.toLowerCase().includes(q.toLowerCase()) || w.shopName.toLowerCase().includes(q.toLowerCase()) || String(w.id).includes(q))
  );
              ...
              {filtered.map((w) => (
                <tr key={w.id}>
                  ...
                  <td className="px-4 py-2 font-medium text-gray-900">
                    <Link href={`/shops/${w.shopId}`} className="hover:text-primary transition-colors">
                      {w.shopName}
                    </Link>
                  </td>
